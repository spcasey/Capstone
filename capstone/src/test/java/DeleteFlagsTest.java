...
public class DeleteFlagsTest {
    ...
    public void properlyDeletes() throws Throwable {
        ...
        Entity entry = dataServlet.addFlagToDatastore("testName", "testAddress", "testLat",
            "testLng", "testUserId", expired);
        Entity anotherEntry = dataServlet.addFlagToDatastore("anotherName", "anotherAddress", "anotherLat",
            "anotherLng", "anotherUserId", new Date());
        ArrayList<Flag> flagList = dataServlet.fetchFlags();
        Assert.assertTrue(flagList.size() == 2);
        DeleteFlags underTest = new DeleteFlags();
        underTest.deleteExpiredFlags();
        flagList = dataServlet.fetchFlags();
        Assert.assertTrue(flagList.size() == 1);
    }
}
